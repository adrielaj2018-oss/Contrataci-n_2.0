...
# =============================
# MODO SOLO GESTIÓN CONTRATACIÓN
# =============================
@app.before_request
def solo_gestion_contratacion():
    """Deja activo solo Gestión de Contratación y bloquea Documental/Vacacional."""
    ruta = request.path.lower().rstrip("/")
    if ruta == "/admin":
        return redirect(url_for("admin_contratacion"))
    if ruta == "/panel":
        if session.get("admin_id"):
            return redirect(url_for("admin_contratacion"))
        return redirect(url_for("trabajador_contratacion") if session.get("dni") else url_for("login"))

    rutas_bloqueadas = (
        "/admin/modulo/documentos",
        "/admin/documentos",
        "/admin/vacaciones",
        "/vacaciones",
        "/documentos",
        "/subir_personal",
        "/documento",
        "/admin/plantilla_gestion/documental",
        "/admin/plantilla_gestion/vacacional",
    )
    if ruta.startswith(rutas_bloqueadas):
        flash("Solo está habilitado el módulo Gestión de Contratación.", "ok")
        return redirect(url_for("admin_contratacion") if session.get("admin_id") else url_for("trabajador_contratacion"))
...
init_db()
asegurar_plantillas_contratacion_base()  # módulo Gestión Contratación activo
...
def sidebar(active):
    ...
    cls_con = 'menu-item active' if active_type == 'Gestion Contratacion' and not active_sub else 'menu-item'
    cls_arch = 'menu-item sub-mini active' if active_sub == 'documentaria' else 'menu-item sub-mini'
    cls_plan = 'menu-item sub-mini active' if active_sub == 'plantillas' else 'menu-item sub-mini'
    cls_flujo = 'menu-item sub-mini active' if active_sub == 'flujo' else 'menu-item sub-mini'
    cls_carga = 'menu-item sub-mini active' if active_sub == 'carga' else 'menu-item sub-mini'
    cls_reportes = 'menu-item sub-mini active' if active_sub == 'reportes' else 'menu-item sub-mini'
    cls_maestros = 'menu-item sub-mini active' if active_sub == 'maestros' else 'menu-item sub-mini'
    cls_anuncio = 'menu-item sub-mini active' if active_sub == 'anuncios' else 'menu-item sub-mini'
    ...
    if session.get('admin_id'):
        admin = f"""
        <div id='grp_admin' data-group='admin' class='menu-group force-open'>
          <button type='button' class='menu-title active' onclick="toggleGroup('grp_admin')">
            <i class='bi bi-grid-1x2'></i><span class='label'>Administrador</span><span class='chev'>∨</span>
          </button>
          <div class='submenu'>
            <div id='grp_contratacion' data-group='contratacion' class='{con_cls}'>
              <button type='button' class='menu-title active' onclick="toggleGroup('grp_contratacion')">
                <i class='bi bi-file-earmark-richtext'></i><span class='label'>Gestión de Contratación</span><span class='chev'>∨</span>
              </button>
              <div class='submenu'>
                <a class='{cls_con}' onclick='saveSideScroll()' href='/admin/contratacion'><i class='bi bi-speedometer2'></i><span class='label'>Dashboard contratación</span></a>
                <a class='{cls_arch}' onclick='saveSideScroll()' href='/admin/contratacion?tab=documentaria'><i class='bi bi-folder2-open'></i><span class='label'>Gestión documentaria</span></a>
                <a class='{cls_plan}' onclick='saveSideScroll()' href='/admin/contratacion?tab=plantillas'><i class='bi bi-file-earmark-word'></i><span class='label'>Plantillas documentos</span></a>
                <a class='{cls_flujo}' onclick='saveSideScroll()' href='/admin/contratacion?tab=flujo'><i class='bi bi-diagram-3'></i><span class='label'>Flujo de aprobación</span></a>
                <a class='{cls_carga}' onclick='saveSideScroll()' href='/admin/contratacion?tab=carga'><i class='bi bi-cloud-arrow-up'></i><span class='label'>Carga masiva</span></a>
                <a class='{cls_reportes}' onclick='saveSideScroll()' href='/admin/contratacion?tab=reportes'><i class='bi bi-bar-chart-line'></i><span class='label'>Reportes</span></a>
                <a class='{cls_maestros}' onclick='saveSideScroll()' href='/admin/contratacion?tab=maestros'><i class='bi bi-sliders'></i><span class='label'>Datos maestros</span></a>
                <a class='{cls_anuncio}' onclick='saveSideScroll()' href='/admin/contratacion?tab=anuncios'><i class='bi bi-megaphone'></i><span class='label'>Anuncio</span></a>
              </div>
            </div>
            <div id='grp_trabajadores_admin' data-group='trabajadores_admin' class='menu-group nested'>
              <button type='button' class='menu-title' onclick="toggleGroup('grp_trabajadores_admin')"><i class='bi bi-people'></i><span class='label'>Trabajadores / Usuarios</span><span class='chev'>∨</span></button>
              <div class='submenu'>
            ...
            <a class='menu-item' onclick='saveSideScroll()' href='/admin/firma/configuracion'><i class='bi bi-pen'></i><span class='label'>Firma digital</span></a>
            <a class='{cls_test}' onclick='saveSideScroll()' href='/admin/modo_prueba'><i class='bi bi-magic'></i><span class='label'>Modo prueba</span></a>
          </div>
        </div>"""
    ...
    if not session.get('admin_id'):
        ...
        <div id='grp_user_contratacion' data-group='user_contratacion' class='menu-group force-open'>
          <button type='button' class='menu-title active' onclick="toggleGroup('grp_user_contratacion')">
            <i class='bi bi-file-earmark-richtext'></i><span class='label'>Gestión Contratación</span><span class='chev'>∨</span>
          </button>
          <div class='submenu'>
            <a class='menu-item active' onclick='saveSideScroll()' href='/contratacion/mis_documentos'><i class='bi bi-folder-check'></i><span class='label'>Mis documentos</span></a>
          </div>
        </div>"""
    ...
    <nav>
      ...
      <div id='grp_cuenta' data-group='cuenta' class='menu-group'>
        <button type='button' class='menu-title' onclick="toggleGroup('grp_cuenta')"><i class='bi bi-person-circle'></i><span class='label'>Mi cuenta</span><span class='chev'>∨</span></button>
        <div class='submenu'>
...
def login_template(admin=False, error=""):
    ...
    if admin:
        ...
        fields = f"""
          <div class='field'><label>Empresa</label><div class='login-input'><i class='bi bi-building'></i><select name='empresa' required><option value=''>Seleccione empresa</option>{opts}</select></div></div>
          <div class='field'><label>Usuario</label><div class='login-input'><i class='bi bi-person'></i><input name='dni' maxlength='8' placeholder='Ingrese su usuario / DNI' required></div></div>
