  ...
  .dash-hero h1{font-size:30px!important;}
    ...
    if sec=='dashboard':
        ...
        <div class='req-pro-grid'>
          ...
          <form method='post' class='pro-card nice-form' id='form_scan_req'><input type='hidden' name='accion' value='registrar_dni_requerimiento'><h3 class='pro-section-title'>2) Escaneo masivo DNI / código de barras al ticket</h3><b>Ticket activo</b><select name='ticket_req' required><option value=''>Seleccione ticket</option>{req_options}</select><b>Modo lectura</b><select id='modo_scan_req'><option>Lector USB / teclado</option><option>Cámara celular / web</option><option>Código de barras DNI</option><option>Digitación manual</option></select><b>DNI / código</b><input id='dni_scan_req' name='dni_scan' required maxlength='12' autofocus placeholder='Escanee o digite DNI y presione ENTER'><b>Resultado</b><input id='scan_result_req' readonly value='Nuevo: guarda DNI / Reingresante: jala datos'><div class='full scan-box scan-box-pro'><div class='scan-camera'><video id='videoScanReq' autoplay playsinline muted></video><canvas id='canvasScanReq' style='display:none'></canvas><span id='scanCamMsg'>Cámara lista para escaneo masivo. Presiona <b>Activar cámara</b> y permite el acceso del navegador.</span></div><div class='scan-tools'><button type='button' class='c-btn gray' onclick='activarCamaraReq()'>📷 Activar cámara</button><button type='button' class='c-btn' onclick='capturarCamaraReq()'>📸 Capturar lectura</button><button type='button' class='c-btn' onclick='agregarDniLocalReq()'>📡 Lectura masiva</button><button type='button' class='c-btn gray' onclick='apagarCamaraReq()'>⏹ Detener</button><button type='button' class='c-btn' onclick='document.getElementById("form_scan_req").requestSubmit()'>💾 Guardar en base</button></div><div class='scan-counter'><span id='cntLeidos'>Leídos: 0</span><span>Nuevo: DNI base</span><span>Reingreso: datos enlazados</span></div><div id='listaScanReq' class='mini-list'></div><p class='muted2'>El lector USB escribe el DNI como teclado. La cámara queda habilitada para vista previa/captura; para lectura automática real de barras/QR se puede conectar librería JS o servicio local.</p></div></form>
        </div>
        ...
        function agregarDniLocalReq(){{const i=document.getElementById('dni_scan_req'); const dni=limpiarDniReq(i.value); if(dni.length!==8){{alert('DNI debe tener 8 dígitos.');return;}} i.value=dni; scanReqCount++; document.getElementById('cntLeidos').innerText='Leídos: '+scanReqCount; document.getElementById('scan_result_req').value='DNI '+dni+' agregado. Si existe en base, queda enlazado como REINGRESANTE''; const box=document.getElementById('listaScanReq'); const row=document.createElement('div'); row.innerHTML='<b>'+dni+'</b><span>Listo para guardar / completar ficha</span><small>'+new Date().toLocaleTimeString()+'</small>'; box.prepend(row); i.select();}}
        async function activarCamaraReq(){{try{{scanReqStream=await navigator.mediaDevices.getUserMedia({{video:{{facingMode:'environment'}},audio:false}}); const v=document.getElementById('videoScanReq'); v.srcObject=scanReqStream; v.style.display='block'; document.getElementById('scanCamMsg').style.display='none';}}catch(e){{alert('No se pudo activar cámara. Abre el sistema en HTTPS/Render o usa localhost/127.0.0.1. También puedes continuar con lector USB.')}}}}
        function capturarCamaraReq(){{const v=document.getElementById('videoScanReq'), c=document.getElementById('canvasScanReq'); if(!v || !v.srcObject || !v.videoWidth){{alert('Primero activa la cámara.');return;}} c.width=v.videoWidth; c.height=v.videoHeight; c.getContext('2d').drawImage(v,0,0,c.width,c.height); const box=document.getElementById('listaScanReq'); const row=document.createElement('div'); row.innerHTML='<b>Captura cámara</b><span>Vista previa tomada. Digita/escanea el DNI y presiona Lectura masiva.</span><small>'+new Date().toLocaleTimeString()+'</small>'; box.prepend(row); document.getElementById('dni_scan_req').focus();}}
        function apagarCamaraReq(){{if(scanReqStream){{scanReqStream.getTracks().forEach(t=>t.stop()); scanReqStream=null;}} const v=document.getElementById('videoScanReq'); if(v){{v.srcObject=null;}} const m=document.getElementById('scanCamMsg'); if(m)m.style.display='block';}}
        </script>
        ''')
    elif sec=='nuevos':
...
.side{width:260px!important;min-width:260px!important;height:100dvh!important;min-height:100dvh!important;background:#083f30!important;padding-bottom:0!important;}
.side.collapsed{width:76px!important;min-width:76px!important;}
.side nav{padding:6px 10px 14px!important;}
.side::after{display:none!important;min-height:0!important;height:0!important;}
.side-user{margin:10px 14px 12px!important;}
.menu-title,.menu-item{max-width:100%!important;min-height:42px!important;padding:9px 11px!important;border-radius:14px!important;}
.menu-title i,.menu-item i{width:28px!important;min-width:28px!important;height:28px!important;}
.menu-title .label,.menu-item .label{font-size:13px!important;line-height:1.12!important;}
.main{padding-left:18px!important;}
#form_scan_req .scan-box,#form_scan_req .scan-box-pro{grid-column:1/-1!important;display:grid!important;grid-template-columns:minmax(360px,1fr) 220px!important;gap:12px!important;align-items:start!important;overflow:visible!important;padding:14px!important;}
#form_scan_req .scan-camera{min-height:250px!important;height:250px!important;padding:0!important;overflow:hidden!important;background:#111827!important;border-radius:16px!important;position:relative!important;}
#form_scan_req .scan-camera video{display:block!important;width:100%!important;height:250px!important;min-height:250px!important;object-fit:cover!important;background:#111!important;border-radius:16px!important;}
#form_scan_req .scan-camera span{position:absolute!important;inset:0!important;display:grid!important;place-items:center!important;text-align:center!important;padding:18px!important;background:#eef3f7!important;color:#3e4a5c!important;font-weight:800!important;line-height:1.25!important;}
#form_scan_req .scan-tools{display:grid!important;grid-template-columns:1fr!important;gap:9px!important;width:100%!important;align-content:start!important;overflow:visible!important;}
#form_scan_req .scan-tools .c-btn{width:100%!important;min-height:46px!important;padding:10px 12px!important;line-height:1.15!important;white-space:normal!important;display:flex!important;align-items:center!important;justify-content:center!important;text-align:center!important;border-radius:14px!important;overflow:visible!important;}
#form_scan_req .scan-counter{grid-column:1/-1!important;display:flex!important;gap:8px!important;flex-wrap:wrap!important;margin-top:2px!important;}
#form_scan_req .mini-list{grid-column:1/-1!important;max-height:150px!important;overflow:auto!important;}
.ingreso-form .camera-box{grid-column:1/-1!important;display:grid!important;grid-template-columns:minmax(300px,1fr) 220px!important;gap:14px!important;align-items:start!important;overflow:visible!important;}
.ingreso-form .camera-box video,.ingreso-form .camera-box img{height:280px!important;min-height:280px!important;max-height:280px!important;}
.ingreso-form .cam-actions{display:grid!important;grid-template-columns:1fr!important;gap:10px!important;width:100%!important;}
.ingreso-form .cam-actions .c-btn{width:100%!important;min-height:46px!important;display:flex!important;align-items:center!important;justify-content:center!important;white-space:normal!important;text-align:center!important;}
.ingreso-form .bio-label{grid-column:1/-1!important;justify-content:flex-start!important;text-align:left!important;margin-top:4px!important;}
.ingreso-form .bio-box{grid-column:1/-1!important;display:grid!important;grid-template-columns:1fr!important;gap:10px!important;overflow:visible!important;}
.ingreso-form .bio-actions{display:grid!important;grid-template-columns:repeat(2,minmax(160px,1fr))!important;gap:10px!important;}
.ingreso-form .bio-actions .c-btn{width:100%!important;min-height:46px!important;white-space:normal!important;}
@media(max-width:1050px){.app,.app.side-collapsed{grid-template-columns:1fr!important}.side{position:fixed!important;left:-280px!important;width:260px!important;min-width:260px!important}.side.open{left:0!important}#form_scan_req .scan-box,#form_scan_req .scan-box-pro,.ingreso-form .camera-box{grid-template-columns:1fr!important}.ingreso-form .bio-actions{grid-template-columns:1fr!important}.main{padding-left:14px!important}}
'''
...
# =========================================================
# PARCHE FINAL UI solicitado por usuario:
# panel fijo con scroll, formularios compactos, cámara/huellero,
# lectura masiva y botones visibles.
# =========================================================
FINAL_UI_PATCH_CSS = '\n/* =========================================================\n   PARCHE FINAL COMPACTO - PANEL FIJO + FORMULARIOS SIN HUECOS\n   ========================================================= */\nhtml,body{margin:0!important;min-height:100%!important;background:#f4f8f6!important;}\n.app{display:grid!important;grid-template-columns:300px minmax(0,1fr)!important;align-items:stretch!important;min-height:100vh!important;background:#f4f8f6!important;}\n.app.side-collapsed{grid-template-columns:84px minmax(0,1fr)!important;}\n.side{\n  position:sticky!important;top:0!important;align-self:start!important;\n  width:300px!important;height:100vh!important;min-height:100vh!important;\n  overflow-y:auto!important;overflow-x:hidden!important;\n  background:linear-gradient(180deg,#0b4a32 0%,#073b2d 55%,#0b4a32 100%)!important;\n  border-right:1px solid rgba(255,255,255,.14)!important;\n}\n.side.collapsed{width:84px!important;}\n.main{min-width:0!important;padding:22px 26px 34px!important;}\n.side-head-pro{position:sticky!important;top:0!important;z-index:20!important;margin:0!important;border-radius:0!important;background:#0b4a32!important;border-left:0!important;border-right:0!important;}\n.side nav{padding-bottom:110px!important;}\n.menu-title,.menu-item{max-width:calc(100% - 20px)!important;}\n.menu-title .label,.menu-item .label{white-space:normal!important;line-height:1.15!important;}\n.side-user{margin:14px 16px 20px!important;}\n/* evita que quede una banda blanca pegada al panel */\n.side::after{content:"";display:block;min-height:180px;background:transparent!important;}\n\n/* formularios más compactos y alineados */\n.c-card,.pro-card,.dash-card{border-radius:18px!important;}\n.c-form,.pro-form,.ingreso-form,.pro-card.nice-form,.learning-grid .nice-form{\n  grid-template-columns:170px minmax(220px,1fr) 170px minmax(220px,1fr)!important;\n  gap:12px 14px!important;\n  padding:20px!important;\n}\n.c-form b,.pro-form b,.ingreso-form b,.pro-card.nice-form b,.learning-grid .nice-form b,.eval-grid b{\n  min-height:44px!important;padding:9px 12px!important;font-size:14px!important;border-radius:12px!important;\n}\n.c-form input,.c-form select,.c-form textarea,.pro-form input,.pro-form select,.pro-form textarea,\n.ingreso-form input,.ingreso-form select,.ingreso-form textarea,.pro-card.nice-form input,.pro-card.nice-form select,.pro-card.nice-form textarea{\n  min-height:46px!important;font-size:14px!important;border-radius:12px!important;\n}\n.pro-section-title,.section-head{padding:11px 14px!important;font-size:17px!important;border-radius:12px!important;margin-bottom:6px!important;}\n.req-pro-grid,.learning-grid{gap:16px!important;}\n/* Nuevos/Reingresantes: cámara + huellero sin espacio muerto */\n.ingreso-form .camera-box{\n  grid-column:2 / 3!important;\n  display:grid!important;grid-template-columns:minmax(240px,1fr) 190px!important;\n  gap:12px!important;align-items:start!important;padding:14px!important;border:1px dashed #b7d4ec!important;border-radius:16px!important;background:#fbfdff!important;\n}\n.ingreso-form .camera-box video,.ingreso-form .camera-box img{\n  width:100%!important;min-height:230px!important;max-height:260px!important;border-radius:14px!important;background:#111!important;object-fit:cover!important;\n}\n.ingreso-form .cam-actions{display:grid!important;grid-template-columns:1fr!important;gap:10px!important;margin:0!important;}\n.ingreso-form .cam-actions .c-btn{width:100%!important;min-height:44px!important;padding:10px!important;white-space:normal!important;}\n.ingreso-form .camera-box small{grid-column:1/-1!important;text-align:center!important;font-size:13px!important;}\n.ingreso-form .bio-box{\n  grid-column:4 / 5!important;\n  display:grid!important;grid-template-columns:1fr!important;\n  gap:12px!important;padding:14px!important;border:1px dashed #b7d4ec!important;border-radius:16px!important;background:#fbfdff!important;\n}\n.ingreso-form .bio-actions{display:grid!important;grid-template-columns:1fr 1fr!important;gap:10px!important;}\n.ingreso-form .bio-actions .c-btn{width:100%!important;min-height:44px!important;padding:10px!important;white-space:normal!important;}\n/* Requerimientos: cámara y lectura masiva visible */\n#form_scan_req .scan-box{\n  grid-column:1/-1!important;display:grid!important;\n  grid-template-columns:minmax(280px,1fr) 230px!important;gap:14px!important;\n  padding:16px!important;border:1px dashed #a7f3d0!important;border-radius:16px!important;background:#f6fffb!important;\n}\n#form_scan_req .scan-camera{min-height:210px!important;border-radius:16px!important;display:grid!important;place-items:center!important;text-align:center!important;padding:16px!important;}\n#form_scan_req .scan-camera video{width:100%!important;height:210px!important;object-fit:cover!important;border-radius:14px!important;background:#111!important;}\n#form_scan_req .scan-tools{display:grid!important;grid-template-columns:1fr!important;gap:10px!important;}\n#form_scan_req .scan-tools .c-btn{width:100%!important;min-height:44px!important;white-space:normal!important;}\n#form_scan_req .scan-counter{grid-column:1/-1!important;display:flex!important;gap:10px!important;flex-wrap:wrap!important;}\n#form_scan_req .scan-counter span{background:#ecfdf5!important;color:#047857!important;border:1px solid #a7f3d0!important;border-radius:999px!important;padding:8px 12px!important;font-weight:800!important;}\n/* Inducción/Cursos: asignación masiva y botones de selección visibles */\n.learning-grid.solo-induccion,.learning-grid.con-evaluacion{grid-template-columns:minmax(0,1fr)!important;}\n.learning-grid .eval-side{width:100%!important;}\n.records-toolbar,.mass-actions,.video-base-grid{display:flex!important;gap:10px!important;align-items:center!important;flex-wrap:wrap!important;}\n.records-toolbar input{flex:1 1 260px!important;min-width:220px!important;}\n.records-toolbar .c-btn,.mass-actions .c-btn{min-height:42px!important;padding:10px 16px!important;border-radius:12px!important;}\n#tabla_capacitacion input[type=checkbox], .c-table input[type=checkbox]{\n  width:20px!important;height:20px!important;accent-color:#10b981!important;cursor:pointer!important;\n}\n.video-base-grid{display:grid!important;grid-template-columns:repeat(4,minmax(130px,1fr))!important;}\n.video-base-grid>div{background:#f8fafc!important;border:1px solid #dbeafe!important;border-radius:14px!important;padding:12px!important;}\n.video-base-grid b{display:block!important;font-size:22px!important;background:transparent!important;padding:0!important;color:#047857!important;}\n.c-table th,.c-table td{font-size:13px!important;padding:10px!important;}\n.clean-table,.c-table{min-width:1050px!important;}\n@media(max-width:1000px){\n  .app,.app.side-collapsed{grid-template-columns:1fr!important;}\n  .side{position:fixed!important;left:-320px!important;width:300px!important;height:100dvh!important;z-index:100!important;}\n  .side.open{left:0!important;}\n  .main{padding:14px!important;}\n  .c-form,.pro-form,.ingreso-form,.pro-card.nice-form,.learning-grid .nice-form,.learning-grid .eval-grid{grid-template-columns:1fr!important;}\n  .ingreso-form .camera-box,.ingreso-form .bio-box,#form_scan_req .scan-box{grid-column:1!important;grid-template-columns:1fr!important;}\n  .video-base-grid{grid-template-columns:1fr 1fr!important;}\n}\n'

@app.after_request
def aplicar_parche_ui_final(response):
    try:
        if response.content_type and 'text/html' in response.content_type.lower():
            html_txt = response.get_data(as_text=True)
            bloque = "<style id='final-ui-patch'>" + FINAL_UI_PATCH_CSS + "</style>"
            if "final-ui-patch" not in html_txt:
                if "</head>" in html_txt:
                    html_txt = html_txt.replace("</head>", bloque + "</head>", 1)
                elif "</body>" in html_txt:
                    html_txt = html_txt.replace("</body>", bloque + "</body>", 1)
                else:
                    html_txt += bloque
