...
.compact-form{display:grid!important;grid-template-columns:170px minmax(180px,1fr) 170px minmax(180px,1fr)!important;gap:12px 18px!important;align-items:center!important}.compact-form textarea{min-height:76px}.compact-form b{font-size:13px;color:#334155!important}.table-wrap{overflow:auto!important}.icon-btn{border:0;background:#ecfdf5!important;color:#047857!important;border-radius:10px;padding:7px 9px;cursor:pointer}.icon-btn:hover{background:#bbf7d0!important}.c-form input[type=file]{padding:9px!important}@media(max-width:900px){.compact-form{grid-template-columns:1fr!important}.compact-form b{margin-top:6px}}</style>"""
    ...
    if sec=='dashboard':
        ...
        req_rows=''.join([f'''<tr><td><b>{h(r['ticket'])}</b></td><td>{h(r['empresa'])}</td><td>{h(r['sede'])}</td><td>{h(r['area'])}</td><td>{h(r['actividad'])}</td><td>{h(r['fecha_ingreso'])}</td><td><span class="status-pill ok">{h(r['estado'])}</span></td><td><form method="post" onsubmit="return confirm(\'¿Eliminar ticket?\')"><input type="hidden" name="accion" value="eliminar_requerimiento"><input type="hidden" name="req_id" value="{r['id']}"><button class="icon-btn action-delete" title="Eliminar">🗑</button></form></td><td>{h(r['responsable'])}</td></tr>''' for r in requerimientos]) or "<tr><td colspan='9'>Sin requerimientos registrados.</td></tr>"
        content=wrap(f"""
        <h2 class='c-title'>Requerimientos / Tickets de personal</h2>
        <div class='dash-hero' style='margin-bottom:18px'><div><h1>Ticket de contratación</h1><p class='muted2'>Crea el requerimiento por sede, área y actividad. Luego escanea DNI/código de barras aquí mismo para agregar trabajadores al ticket.</p></div><a class='c-btn' href='/admin/contratacion?sec=nuevos'>Completar ficha trabajador</a></div>
        <div class='split-two req-layout'>
          <form method='post' class='c-card c-form pro-form' style='padding:20px'><input type='hidden' name='accion' value='guardar_requerimiento'><h3 class='section-head'>1) Datos del ticket</h3><span></span><b>Ticket</b><input name='ticket' required placeholder='REQ-2026-0001 / LABORES 29.08.2024'><b>Empresa</b><input name='empresa' value='AQUANQA' required><b>Sede</b><input name='sede' required placeholder='Blueberries / Arato / Olmos'><b>Área</b><input name='area' required placeholder='Campo / Packing / RRHH'><b>Actividad</b><input name='actividad' required placeholder='OB_PODA / COSECHA'><b>Fecha ingreso objetivo</b><input type='date' name='fecha_ingreso' value='{hoy_iso()}' required><b>Prioridad</b><select name='prioridad'><option>ALTA</option><option selected>MEDIA</option><option>BAJA</option></select><b>Estado</b><select name='estado'><option>SOLICITADO</option><option>APROBADO</option><option>EN CONVOCATORIA</option><option>EN REGISTRO</option><option>EN PROCESO</option><option>CERRADO</option></select><b>Responsable</b><input name='responsable' placeholder='Responsable RRHH'><b>Observación</b><textarea name='observacion' rows='2' placeholder='Detalle del requerimiento'></textarea><input type='hidden' name='cargo' value='POR DEFINIR'><input type='hidden' name='cantidad' value='0'><span></span><button class='c-btn'>💾 Crear ticket</button></form>
          <form method='post' class='c-card c-form pro-form' style='padding:20px'><input type='hidden' name='accion' value='registrar_dni_requerimiento'><h3 class='section-head'>2) Escaneo DNI / código de barras al requerimiento</h3><span></span><b>Ticket activo</b><select name='ticket_req' required><option value=''>Seleccione ticket</option>{req_options}</select><b>DNI / código</b><input id='dni_scan_req' name='dni_scan' required maxlength='12' autofocus placeholder='Escanee o digite DNI'><b>Modo lectura</b><select><option>Lector USB / teclado</option><option>Cámara celular / web</option><option>Código de barras DNI</option></select><b>Resultado</b><input readonly value='Nuevo: guarda DNI / Reingresante: jala datos'><span></span><button class='c-btn'>➕ Agregar al ticket</button><p class='muted2'>El lector de barras normalmente escribe el número y presiona ENTER. Esta base queda enlazada con Nuevos/Reingresantes, Médico, Indumentaria, Fotocheck y NISIRA.</p></form>
        </div>
        ...
        content=wrap(f"""
        <h2 class='c-title'>{'Inducción' if sec=='induccion' else ('Cursos / Capacitación' if sec=='cursos' else 'Inducción y cursos')}</h2><div class='dash-hero' style='margin-bottom:18px'><div><h1>{'Inducción laboral' if sec=='induccion' else ('Cursos y capacitación' if sec=='cursos' else 'Inducción y cursos')}</h1><p class='muted2'>Administrador carga videos y asigna cursos; la evaluación queda para el usuario trabajador en celular.</p></div><a class='c-btn' href='/admin/contratacion?sec=firma'>Ir a firma</a></div>
        <div class='split-two'><form id='form_capacitacion' method='post' enctype='multipart/form-data' class='c-card c-form pro-form' style='padding:20px'><input type='hidden' name='accion' value='guardar_capacitacion'><h3 class='section-head'>1) Biblioteca de inducción / cursos - Administrador</h3><span></span><b>Curso</b><select name='curso' required><option>Inducción general</option><option>SST</option><option>Reglamento interno</option><option>Buenas prácticas agrícolas</option><option>Uso de EPP</option><option>Código de conducta</option><option>Manipulación de alimentos</option><option>Bioseguridad</option></select><b>Tipo video</b><select name='tipo_video' required><option>URL EXTERNA</option><option>ARCHIVO MP4</option><option>YOUTUBE / DRIVE</option><option>MICROLEARNING</option></select><b>URL video</b><input name='video_url' placeholder='https://...'><b>Cargar video MP4</b><input type='file' name='archivo_video' accept='.mp4,.webm,.mov'><b>Duración min.</b><input name='duracion_min' type='number' min='0' placeholder='0'><b>Fecha inicio</b><input type='date' name='fecha_inicio' value='{hoy_iso()}' required><b>Fecha fin</b><input type='date' name='fecha_fin' value='{hoy_iso()}'><h3 class='section-head'>2) Asignación al trabajador</h3><span></span><b>DNI</b><input name='dni' list='lista_ingresos' maxlength='8' required placeholder='Digite DNI y seleccione trabajador'><datalist id='lista_ingresos'>{opt_ingresos}</datalist><b>Estado</b><select name='estado' required><option>PENDIENTE</option><option>VIDEO ASIGNADO</option><option>VIDEO VISTO</option><option>EVALUADO POR TRABAJADOR</option><option>APROBADO</option><option>DESAPROBADO</option></select><b>Aprobador</b><input name='aprobador' placeholder='Responsable SST/RRHH'><b>Evidencia</b><input type='file' name='evidencia' accept='.pdf,.png,.jpg,.jpeg,.xlsx'><b>Observación</b><textarea name='observacion' rows='2' placeholder='Observación del administrador.'></textarea><span></span><button class='c-btn'>💾 Guardar video/asignación</button></form><div class='c-card worker-eval'><h3>Evaluación del trabajador</h3><p>Estos campos quedan preparados para el usuario trabajador: nota, respuestas, intentos, evidencia y confirmación de video visto.</p><div class='eval-grid'><b>Nota / resultado</b><input name='nota' form='form_capacitacion' placeholder='Ej. 18/20'><b>Preguntas evaluación</b><input name='preguntas' form='form_capacitacion' placeholder='Cantidad o link de evaluación'><b>Intentos permitidos</b><input name='intentos' form='form_capacitacion' type='number' min='1' value='1'><b>Confirmación</b><select disabled><option>El trabajador confirma desde su portal</option></select></div></div></div>
        {bandeja_operativa('tabla_embudo_capacitacion')}
